			...
			if ($this->method == 'GET') {
				...
				if (empty($this->params)) {
					...
					if (empty($this->params['basket_id'])) {
						...
						$con->query("select basket.basket_id, rooms.room_id, rooms.noroom, hotel.hotel_name,hotel.image,roomtypes.rent,request.status,request.key,request.checkin,request.checkout from basket
						INNER JOIN rooms on rooms.room_id=basket.room_id 
						INNER JOIN roomtypes on roomtypes.roomtype_id=rooms.roomtype_id 
						INNER JOIN hotel on hotel.id=roomtypes.hotel_id 
						INNER JOIN request on request.id=basket.id_request where basket.user_id='$user_id'");
				...
				if (empty($id)) {
					...
					// else{	
					// 	$user_id = $check['user_id'];
					$con->query("DELETE FROM basket WHERE basket_id = '$id'");
					$con->close();
